                ...
                <div class='row'>
		            ...
		            {{ f.campground_name.slice(0, 20) }}<span v-if="f.campground_name.length > 20">...</span>
                       ...
                       <!-- This line has to be changed to use a v-if/else clause
                        ...
                        -->
                       <a v-if="f.campground_type == 0" class="button formButton1" style="width:100%;" v-bind:href="parkstayUrl+'/availability/?site_id='+f.id+'&'+bookingParam" target="_self">Book now</a>
                       <a v-else-if="f.campground_type == 1" class="button formButton" style="width:100%;" v-bind:href="f.info_url" target="_blank">More Info</a>
                       <a v-else class="button formButton2" v-bind:href="f.info_url" style="width:100%;" target="_blank">More info</a>
                   </div>
               </div>
            </div>
	</div>
        ...
        this.olmap.on('singleclick', function(ev) {
            var result = ev.map.forEachFeatureAtPixel(ev.pixel, function(feature, layer) {
                ...
                if (feature.get('images')) {
                    ...
                    $("#mapPopupImage").attr('src', vm.parkstayUrl+'/campground-image/146/248/?mediafile='+popup_image);
                    $("#mapPopupImage").show();
                } else {
                    $("#mapPopupImage").hide();
                }
