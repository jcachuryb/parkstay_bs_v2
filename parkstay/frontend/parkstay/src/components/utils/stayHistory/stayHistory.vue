<template id="stayHistory">
    <div class="row">
        <StayHistoryDetail :stay="stay" :campground="campground" ref="addMaxStayModal"
            @addCgStayHistory="addStayHistory()" @updateStayHistory="updateStayHistory()"></StayHistoryDetail>
        <div class="well">
...
const msh_headers = ref(['ID', 'Period Start', 'Period End', 'Maximum Stay(Nights)', 'Reason', 'Comment', 'Action'])
const msh_options = ref({
    ...
    ajax: {
        url: api_endpoints.campgroundStayHistory(props.object_id),
        dataSrc: ''
    },
    columns: [{
        "data": "id"
...
const addStayHistory = function () {
    sendData(api_endpoints.campground_stay_history, 'POST')
}
const updateStayHistory = function () {
    sendData(api_endpoints.campground_stay_history_detail(stay.value.id), 'PUT');
}
...
const sendData = function (url, method) {
    var data = stay.value;
    if (method == 'POST') {
        data.campground = props.object_id;
    }
    $.ajax({
        url: url,
        method: method,
        ...
        error: function (resp) {
            retrieve_stay.value.error = true;
            retrieve_stay.value.errorString = helpers.apiError(resp);
        }
    });
