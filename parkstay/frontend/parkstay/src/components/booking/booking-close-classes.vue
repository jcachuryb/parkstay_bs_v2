                                            ...
                                            <table class="hover table table-striped table-bordered dt-responsive nowrap"
                                                ...
                                                <tbody><template v-for="c in booking.campsite_classes">
                                                        <tr>
                                                            ...
                                                            <td class="numBook">
                                                                <input class="checkbox" type="checkbox" :value="c.id"
                                ...
                                <div class="row">
                                    <div class="col-lg-12">
                                        <h3 class="text-primary" v-if="park.entry_fee_required">Park Entry Fees
                                            <small>(${{ $filters.formatMoney(parkPrices.vehicle) }}/per vehicle)</small>
                                        </h3>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="input-group">
                                                ...
                                                <input type="text" class="form-control"
                                                    :placeholder="$filters.formatMoney(0)"
                                                    v-bind:value="$filters.formatMoney(booking.price)" readonly="true">
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="input-group" v-if="overrideCharge">
                                                ...
                                                <input type="text" class="form-control" name="overridePrice"
                                                    :placeholder="$filters.formatMoney(0)" v-model="booking.override_price">
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="col-lg-12">
                                        <div class="form-group">
                                            <input type="checkbox" name="sendInvoice" class="form control"
                                                id="send_invoice" checked :checked="checked"
                                                v-model="booking.send_invoice" />
...
const bookingForm = ref(route.params.id)
const countries = ref([])
...
const selectAll = computed({
    get: function () {
        return booking.value.campsite_classes ? multibook_selected.value.length == booking.value.campsite_classes.length : false;
    },
    set: function (value) {
        var selected = [];

        if (value) {
            booking.value.campsite_classes.forEach(function (campsite) {
...
const addEventListeners = function () {
    ...
    arrivalPicker.value = getDateTimePicker(arrivalPickerElement, {
        restrictions: {
            ...
            maxDate: dateUtils.addDays(new Date(), parseInt(campground.value.max_advance_booking))
        }
    });
    ...
    fetch(api_endpoints.campgroundCampsites(campground.value.id)).then((response) => response.json()).then((data) => {
        var campsites = data;
        fetch(api_endpoints.campsite_current_price(campsites[0].id, Moment().format("YYYY-MM-DD"), Moment().add(1, 'days').format("YYYY-MM-DD"))).then((response) => response.json()).then((data) => {
            priceHistory.value = null;
            priceHistory.value = data;
...
const generateBookingPrice = function () {
    ...
    if (park.value.entry_fee_required) {
        fetchParkPrices(function () {
            $.each(priceHistory, function (i, price) {
...
const fetchParkPrices = function (calcprices) {
    if (booking.value.arrival) {
        fetch(api_endpoints.park_current_price(park.value.id, booking.value.arrival)).then((response) => response.json()).then((data) => {
            var resp = data;
