...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        try:
            ...
            booking_reminder = models.Booking.objects.filter(reminder_email_sent=False, do_not_send_confirmation=False, property_cache__paid=True,error_sending_confirmation=False, arrival__lt=next_check, next_check_for_payment__lt=timezone.now(), error_sending_reminder=False).exclude(booking_type=3).order_by('id')[:30]
